...
rs_allocation gCurrentRGBFrame;  // BLACK
...
uchar4 __attribute__((kernel)) toCartoon(uint32_t x, uint32_t y) {
  ...
  uchar4 rgba;
  uchar4 org = rsGetElementAt_uchar4(gCurrentRGBFrame, x, y);

  rgba.r = clamp(org.r - v * 10, 0, 255);
  rgba.g = clamp(org.g - v * 10, 0, 255);
  rgba.b = clamp(org.b - v * 10, 0, 255);
  rgba.a = 255;
